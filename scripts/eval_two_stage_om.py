#!/usr/bin/env python3
"""Evaluate the two-stage MediaPipe hand pipeline with Ascend OM models."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--landmark", default="models/om/mediapipe_legacy_0_10_14_hand_landmark_full.om")
    ...
    parser.add_argument("--output-dir", default="runs/om_two_stage/legacy_full")
    ...
    parser.add_argument("--reload-landmark-each-call", action="store_true")
...
class OmRunner:
    def __init__(self, model_path: Path, reload_each_call: bool) -> None:
        self.model_path = model_path
        self.reload_each_call = reload_each_call
        self.model: AclOmModel | None = None
        self.load_times_ms: list[float] = []
        self.infer_times_ms: list[float] = []

    def open(self) -> None:
        if not self.reload_each_call and self.model is None:
            start = time.perf_counter()
            self.model = AclOmModel(self.model_path)
            self.load_times_ms.append((time.perf_counter() - start) * 1000.0)
    ...
    def __call__(self, tensor: np.ndarray) -> list[np.ndarray]:
        if self.reload_each_call:
            start = time.perf_counter()
            model = AclOmModel(self.model_path)
            self.load_times_ms.append((time.perf_counter() - start) * 1000.0)
            try:
                infer_start = time.perf_counter()
                outputs = model(tensor)
                self.infer_times_ms.append((time.perf_counter() - infer_start) * 1000.0)
                return outputs
            finally:
                model.close()
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    lines = [
        "# Two-stage OM Hand Pipeline Evaluation",
        "",
        "## Settings",
        "",
        f"- data: `{summary['data']}`",
        ...
        f"- keep_detector_loaded: `{summary['keep_detector_loaded']}`",
        f"- reload_landmark_each_call: `{summary['reload_landmark_each_call']}`",
        "",
    ...
    lines.extend(
        [
            ...
            "| --- | ---: | ---: | ---: |",
            f"| detector load | {fmt(summary['det_load_mean_ms'])} | {fmt(summary['det_load_median_ms'])} | {fmt(summary['det_load_p95_ms'])} |",
            f"| detector call | {fmt(summary['det_call_mean_ms'])} | {fmt(summary['det_call_median_ms'])} | {fmt(summary['det_call_p95_ms'])} |",
            f"| detector inference | {fmt(summary['det_infer_mean_ms'])} | {fmt(summary['det_infer_median_ms'])} | {fmt(summary['det_infer_p95_ms'])} |",
            f"| landmark load | {fmt(summary['landmark_load_mean_ms'])} | {fmt(summary['landmark_load_median_ms'])} | {fmt(summary['landmark_load_p95_ms'])} |",
            f"| landmark call | {fmt(summary['landmark_call_mean_ms'])} | {fmt(summary['landmark_call_median_ms'])} | {fmt(summary['landmark_call_p95_ms'])} |",
            ...
            "Notes:",
            "",
            "- Detector load time is reported separately because palm OM output drifted when one loaded detector handle was reused repeatedly.",
            "- For accuracy, the default mode reloads the detector per image and keeps the landmark model loaded.",
        ]
    )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    det_call_ms: list[float] = []
    det_decode_ms: list[float] = []
    roi_ms: list[float] = []
    landmark_call_ms: list[float] = []
    landmark_post_ms: list[float] = []
    total_ms: list[float] = []
    saved_vis = 0

    with AclRuntime(args.device_id):
        detector = OmRunner(detector_path, reload_each_call=not args.keep_detector_loaded)
        landmark = OmRunner(landmark_path, reload_each_call=args.reload_landmark_each_call)
        detector.open()
        landmark.open()
        try:
            for image_id, image_path in enumerate(image_paths):
                ...
                det_call_ms.append((time.perf_counter() - infer_start) * 1000.0)
                ...
                for hand_index, palm in enumerate(sorted(palms, key=lambda item: item.score, reverse=True)[: args.max_hands]):
                    ...
                    landmark_call_ms.append((time.perf_counter() - lm_start) * 1000.0)
    ...
    summary: dict[str, Any] = {
        "task": "eval_two_stage_om",
        "data": str(data_root),
        "split": args.split,
        "images": len(image_paths),
        "detector": str(detector_path),
        "landmark": str(landmark_path),
        "keep_detector_loaded": args.keep_detector_loaded,
        "reload_landmark_each_call": args.reload_landmark_each_call,
        ...
        **summarize_times(detector.load_times_ms, "det_load"),
        **summarize_times(det_pre_ms, "det_preprocess"),
        **summarize_times(det_call_ms, "det_call"),
        **summarize_times(detector.infer_times_ms, "det_infer"),
        **summarize_times(det_decode_ms, "det_decode"),
        **summarize_times(roi_ms, "roi_preprocess"),
        **summarize_times(landmark.load_times_ms, "landmark_load"),
        **summarize_times(landmark_call_ms, "landmark_call"),
        **summarize_times(landmark.infer_times_ms, "landmark_infer"),
