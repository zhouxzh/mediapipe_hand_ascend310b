...
from hand_pipeline.preprocess import nv12_to_rgb_crop
...
def _cv_border_mode(border_mode: str) -> int:
    if border_mode == "replicate":
        return cv2.BORDER_REPLICATE
    if border_mode in {"zero", "constant"}:
        return cv2.BORDER_CONSTANT
    raise ValueError(f"Unknown border_mode: {border_mode}")


def normalized_rect_projection(
    image_width: int,
    image_height: int,
    rect: NormalizedRect,
    input_size: int = 224,
    ...
    pixel_rect = normalized_rect_to_pixel_dict(rect, image_width, image_height)
    ...
    src = np.asarray(
        [
            center - 0.5 * rect_width * x_axis + 0.5 * rect_height * y_axis,
            center - 0.5 * rect_width * x_axis - 0.5 * rect_height * y_axis,
            center + 0.5 * rect_width * x_axis - 0.5 * rect_height * y_axis,
            center + 0.5 * rect_width * x_axis + 0.5 * rect_height * y_axis,
        ],
    ...
    return matrix, inverse, projection_matrix, center, max(rect_width, rect_height), rotation
...
def hand_roi_from_normalized_rect(
    ...
    matrix, inverse, projection_matrix, center, roi_size, rotation = normalized_rect_projection(
        width,
        height,
        rect,
        input_size=input_size,
    ...
    crop = cv2.warpPerspective(
        ...
        (input_size, input_size),
        flags=cv2.INTER_LINEAR,
        borderMode=_cv_border_mode(border_mode),
    )
    return HandRoi(
        matrix=matrix,
        inverse=inverse,
...
def hand_roi_tensor_from_nv12(
    ...
    crop_rgb = nv12_to_rgb_crop(
        nv12,
        width,
        height,
        projection_matrix,
        input_size,
        border_mode=_cv_border_mode(border_mode),
    )
    tensor = np.ascontiguousarray((crop_rgb.astype(np.float32) / 255.0)[None])
    roi = HandRoi(matrix=matrix, inverse=inverse, center=center, size=roi_size, rotation=rotation, crop=crop_rgb)
    return tensor, roi
...
def associate_normalized_rects(
    ...
    Overlapping rects are suppressed inside each input stream and across later
    input streams. MediaPipe gives later input streams higher priority; in the
    hand tracking graph, previous landmark rects are later than palm rects.
    """
    result: list[NormalizedRect] = []
    for rects in rect_vectors:
        if not rects:
            continue
        if not result:
            result.append(rects[0])
            start_index = 1
        else:
            start_index = 0
        for rect in rects[start_index:]:
